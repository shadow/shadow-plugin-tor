...
static void _torflowaggregator_printToFile(TorFlowAggregator* tfa) {
	...
	while(g_hash_table_iter_next(&iter, &key, &value)) {
		...
		if (current->newBandwidth > (gint)(totalBW * tfa->nodeCap)){
			message("Capping bandwidth for extremely fast relay %s\n",
...
static void _torflowaggregator_readInitialAdvertisements(TorFlowAggregator* tfa) {
	...
	if(tfa->loadedInitial) {
		debug("Already loaded initial advertisements");
		return;
	}
	...
	if (fp == NULL) {
		critical("Could not open v3bw file %s for reading", tfa->filepath);
		return;
	}
	gchar* line = NULL;
	gsize len = 0;
	...
	if (getline(&line, &len, fp) == -1) {
		critical("Error reading from v3bw file %s", tfa->filepath);
		return;
	}
	...
	while((read = getline(&line, &len, fp)) != -1) {
		...
		for(gint i = 0; (rpart = rparts[i]) != NULL; i++) {
			...
			if(!(iparts[0] && iparts[1])) {
				warning("Error parsing token %s from v3bw file %s", rpart, tfa->filepath);
				continue;
				...
				warning("Unrecognized field %s in v3bw file", iparts[0]);
			}
		}
		
		if(!tfrs->identity) {
			warning("No node_id found in line %s in v3bw file", line);
	...
	message("Read %u nodes from file",
...
gint torflowaggregator_loadFromPresets(TorFlowAggregator* tfa, GSList* relays) {
	...
	for(GSList* currentNode = relays; currentNode; currentNode = g_slist_next(currentNode)) {
		...
		if(!stats) {
			warning("Relay %s read in descriptor from torctl port, but not found in initialization file",
					current->identity->str);
			continue;
		}
		debug("for $%s, descriptorBandwidth was %i, advertisedBandwidth was %i",
...
/*
    ...
    fprintf(fp, "%li\n", now_ts.tv_sec);
*/
    message("Slice %u complete. Measurements:", slice->sliceNumber);
    ...
    while (currentNode) {
        ...
        if (current && current->measureCount > 0) {
            debug("Current[0]: Bytes %i Total %i",
...
*/
            message("node_id=%s nick=%s strm_bw=%i filt_bw=%i desc_bw=%i ns_bw=%i\n",
            ...
            message("%s: unmeasured", current && current->nickname ? current->nickname->str : "unknown");
        }
...
void torflowaggregator_reportMeasurements(TorFlowAggregator* tfa, TorFlowSlice* slice, gboolean printFile) {
	...
	while (currentNode) {
		...
		if (current->measureCount >= MEASUREMENTS_PER_SLICE) {
			...
			info("stored new measurements for %s (%s) desc=%i adv=%i mean=%i filtered=%i",
